...
| C2 | **Checkout tạo N đơn** | CU-04 (idempotency, khóa kho, giới hạn đơn mở, quét lười, snapshot giá, snapshot `stall_label`, notify) | C-02, C-03 | **Rất cao** | 12 | 6 |
...
|---|---|---|:---:|:---:|
| P2 | Xác thực | CustomUser manager, roles seed dùng chung với P6; AU-01 (đăng ký Customer), AU-03 → 07 (đăng nhập, refresh, logout, me, đổi mật khẩu); **AU-09 đăng nhập Admin (cổng riêng, D-027)**; `ACCOUNT_LOCKED` kèm lý do khi đúng mật khẩu (D-024); blacklist Redis; throttle; permission class theo role. Màn hình G-09, G-10, **A-00 `/admin/login`** (`AdminAuthLayout`), trang đổi mật khẩu cho 3 role | 8 | 6 |
...
| F6 | Danh sách đơn, đếm theo tab, danh sách soạn hàng | FA-19 → 22 (quét lười) | F-02, F-03 | TB | 6 | 6 |
...
| P6 | Dữ liệu | 24 model, **chạy migration (chỉ Lead)**, data migration 3 role, seed demo (chợ, Farmer, sản phẩm, đơn, review, 3 kỳ nghỉ), xuất `.sql`, SY-01 health | 5 | — |
...
| A3 | Quản lý khách hàng | AD-09 → 13 (khóa → T13; lưu / xóa `deactivation_reason`, D-024) | A-04 (cột Lý do khóa) | TB | 6 | 3 |
