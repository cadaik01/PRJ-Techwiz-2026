...
> Căn cứ: tài liệu thiết kế v1.7 (`MarketLink_requirement_analysis_ok (5).md` — D-001 → D-035, 24 bảng, API gồm FA-31 → FA-35, AD-31 → AD-33).
...
| F7 | **Hành động FSM + lõi FSM + dịch vụ kho + xử lý yêu cầu thay đổi** | FA-23 → 27 (T2 khóa và trừ kho, thiếu → `INSUFFICIENT_STOCK`; T4 tới trước `pickup_start_at`, FA-24 có `mark_sold_out`; T9 chặn khi còn yêu cầu thay đổi; T11 và **T14** cộng trả kho); **FA-34, FA-35** chấp nhận / từ chối yêu cầu thay đổi (D-030); ma trận `TRANSITIONS` v1.7 (bỏ T7, thêm T14), Triple-Gate, OCC, ghi `order_status_history`; `lock_products()`, `apply_stock_delta()`, hàm kiểm tra tồn kho khả dụng; `expire_overdue_orders()` (thêm tự hủy yêu cầu thay đổi quá hạn) | Nút theo trạng thái trên F-02, F-03; khối "Change request" ở F-03 (3 nút: chấp nhận / giữ đơn cũ / hủy cả đơn) | **Rất cao** |
...
| A2 | **Quản lý Farmer** | AD-02 → 08 (xem trước ảnh hưởng, đình chỉ → T3/T4/T12 qua lõi FSM, **chỉ hoàn kho đơn đã duyệt / sẵn sàng** — D-029, xóa yêu cầu thay đổi đang chờ, thông báo; lý do Admin nhập chỉ lưu `status_reason` + `audit_logs`, không vào lịch sử đơn hay email — D-033) | A-02, A-03 | **Cao** |
| A3 | Quản lý khách hàng | AD-09 → 13 (khóa → T5/T6/T13 qua lõi FSM, hoàn kho như A2; lưu / xóa `deactivation_reason`, D-024; **cờ `at_risk` chỉ đếm `NO_SHOW` + bộ lọc (D-028)**) | A-04 (cột Lý do khóa, badge At risk) | TB |
| A4 | **Chợ: CRUD, ngày mở cửa, giờ mở cửa, tọa độ, lịch đóng cửa** | AD-14 → 17 (AD-16 tự tắt khung + `MARKET_SCHEDULE_CHANGED`; AD-17 chặn khi còn đơn mở — D-022); **AD-31 → 33** (D-023) | A-05, A-06 (dùng MapPicker; khối Lịch đóng cửa tạm thời) | **Cao** |
| A5 | Danh mục sản phẩm | AD-18, 19 | A-07 | Thấp |
| A6 | Kiểm duyệt sản phẩm và review | AD-20 → 24 | A-08 | Thấp–TB |
| A7 | Báo cáo + xuất Excel | AD-25, 26 (openpyxl, ghi `EXPORT_DATA`) | A-09 | TB |
| A8 | Thông báo toàn sàn | AD-27, 28 | A-10 | Thấp |
| A9 | Nhật ký hệ thống | AD-29, 30 | A-11 | Thấp |
...
| Lõi FSM `transition_order(...)` + ma trận `TRANSITIONS` v1.7 (kho theo D-029, T14, bỏ T7, tự xóa `pending_change` khi đơn bị hủy / từ chối) | Farmer | Customer (hủy đơn), Admin (T3/T4/T12, T5/T6/T13) | Giữa ngày 2 |
...
3. **Admin chủ trì**: Admin đình chỉ Farmer → đơn mở tự từ chối, chỉ đơn đã duyệt / sẵn sàng được hoàn kho, khách nhận thông báo; Admin đổi lịch chợ → khung của Farmer tự tắt.
