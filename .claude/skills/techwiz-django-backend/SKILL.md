...
description: Quy chuẩn kiến trúc, nguyên lý lập trình, hợp đồng dữ liệu, chữ ký giao diện và bảng tra cứu kỹ thuật cho phân hệ Backend Django REST Framework với CSDL MySQL 8.x (InnoDB), kèm quy ước quyền tác giả commit (không để AI thành contributor). Dùng khi viết hoặc review code backend, và BẮT BUỘC dùng trước khi commit, push hoặc mở Pull Request.
---
...
## 16. Quy Ước Git & Quyền Tác Giả Commit (Contributor Hygiene)
...
### 16.1. GitHub tính contributor từ đâu
...
* **Tuyệt đối không** thêm dòng quảng bá công cụ vào commit message hay mô tả Pull Request (`Generated with ...`, `Generated by ...`, link session, emoji robot).
* **Tuyệt đối không** ghi tên model AI vào comment mã nguồn, docstring, hay bất kỳ file nào được commit.
* **AI Assistant khi commit hộ**: nếu `git config user.name` / `user.email` của môi trường đang là danh tính AI, bắt buộc commit bằng danh tính thành viên đang làm việc, không sửa git config:
  ```bash
  GIT_COMMITTER_NAME="Nguyen Van A" GIT_COMMITTER_EMAIL="email@example.com" \
    git commit --author="Nguyen Van A <email@example.com>" -m "..."
  ```
* **AI Assistant khi tạo Pull Request**: một số công cụ tự chèn dòng `Generated by ...` vào cuối mô tả PR. Sau khi tạo PR, bắt buộc đọc lại mô tả và xóa dòng đó nếu có.
...
> ℹ️ **Phân biệt với Bản Công Bố Sử Dụng AI (Playbook Pass 5, mục 5)**: Việc liệt kê minh bạch công cụ AI đã dùng trong tài liệu nộp Aptech là **bắt buộc** và nằm trong hồ sơ bàn giao. Quy định ở mục này chỉ áp dụng cho **lịch sử Git** (author, committer, trailer) và mô tả Pull Request.

### 16.3. Kiểm tra trước khi push
...
git log --format='%B' \
  | grep -inE '^co-authored-by:|^generated (with|by)|noreply@anthropic|@users\.noreply\.github\.com.*copilot' \
...
### 16.4. Chặn tự động bằng git hook (khuyến nghị)
...
if grep -qiE 'co-authored-by:.*(claude|anthropic|copilot|cursor)|generated (with|by)' "$1"; then
