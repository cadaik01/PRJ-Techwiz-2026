...
def custom_exception_handler(exc: Exception, context: dict[str, Any]) -> Response:
    ...
    if isinstance(exc, DjangoValidationError):
        ...
        return reply("Invalid input. Please check the highlighted fields.", 400,
                     ErrorCode.VALIDATION_ERROR, flatten_errors(detail))

    if isinstance(exc, DRFValidationError):
        return reply("Invalid input. Please check the highlighted fields.", 400,
                     ErrorCode.VALIDATION_ERROR, flatten_errors(exc.detail))
    ...
    if isinstance(exc, APIException):
        return reply(str(exc.detail), exc.status_code,
                     DEFAULT_ERROR_CODES.get(exc.status_code, ErrorCode.VALIDATION_ERROR))
