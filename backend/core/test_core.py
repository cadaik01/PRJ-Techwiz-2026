...
from core.signals import attach_history_request_id
...
def test_client_request_id_is_kept_only_if_it_is_a_uuid(api_client):
    ...
    assert api_client.get(reverse('auth-me'), HTTP_X_REQUEST_ID=supplied)['X-Request-ID'] == supplied

    forged = api_client.get(reverse('auth-me'), HTTP_X_REQUEST_ID='x\nFAKE LOG LINE')
