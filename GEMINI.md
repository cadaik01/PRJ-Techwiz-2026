...
Scope-Aware Testing Rules:
- **Frontend Tasks**: NEVER run backend tests (`pytest`). Use frontend verification (`npm run build` or targeted component check) only.
- **Backend Tasks**: Run targeted tests (`pytest <test_file> -q`) after each logical change.
- Never run the full test suite (900+ tests) unless explicitly requested by the user.
...
A task is considered complete only after the applicable automated verification for the CURRENT SCOPE has been executed:
- For frontend: frontend build / syntax / render verification.
- For backend: targeted pytest for the modified module/feature.
