...
export function useUpdateFarmerMarketStall() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ farmerMarketId, stall_label, }) => farmerApi.updateMarket(farmerMarketId, { stall_label }),
        onSuccess: () => {
...
export function useTogglePickupSlot() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ slotId, is_active }) => farmerApi.updatePickupSlot(slotId, { is_active }),
        onSuccess: () => {
            void invalidateFarmerMarkets(queryClient);
        },
        onError: (e) => toast.error(ApiError.fromUnknown(e).friendlyMessage),
