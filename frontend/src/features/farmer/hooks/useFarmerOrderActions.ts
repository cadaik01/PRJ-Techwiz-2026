...
export function useFarmerOrderActions(
  ...
  const run = useMutation({
    mutationFn: async (payload: { action: FarmerOrderAction; reason: string }) => {
      const { action, reason } = payload;
      const version = order.version;
      if (action === 'ACCEPT') return farmerApi.acceptOrder(order.id, version);
      if (action === 'DECLINE') return farmerApi.declineOrder(order.id, reason, version);
      if (action === 'READY') return farmerApi.readyOrder(order.id, version);
      if (action === 'COMPLETE') return farmerApi.completeOrder(order.id, version);
