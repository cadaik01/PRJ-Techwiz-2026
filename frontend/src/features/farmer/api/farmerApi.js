...
import { adaptFarmerProduct, adaptFarmerProductPage } from '@/lib/adapters/catalog.adapter';
...
export const farmerApi = {
    ...
    acceptOrder: async (id, version) => {
        const { data } = await axiosClient.post(`/orders/${id}/accept/`, {}, { ifMatch: String(version) });
        return data;
    },
    declineOrder: async (id, reason, version) => {
        const { data } = await axiosClient.post(`/orders/${id}/decline/`, { reason }, { ifMatch: String(version) });
        return data;
    },
    readyOrder: async (id, version) => {
        const { data } = await axiosClient.post(`/orders/${id}/ready/`, {}, { ifMatch: String(version) });
        return data;
    },
    completeOrder: async (id, version) => {
        const { data } = await axiosClient.post(`/orders/${id}/complete/`, {}, { ifMatch: String(version) });
        return data;
    },
    noShowOrder: async (id, version) => {
        const { data } = await axiosClient.post(`/orders/${id}/no-show/`, {}, { ifMatch: String(version) });
        return data;
    },
    ...
    createProduct: async (payload) => {
        ...
        if (!product)
            throw new Error('Product response missing id');
        return product;
    },
    ...
    updateMarket: async (farmerMarketId, payload) => {
        const { data } = await axiosClient.patch(`/farmer-markets/${farmerMarketId}/`, payload);
        return data;
    },
    ...
    getReviews: async (params = {}) => {
        const { data } = await axiosClient.get('/reviews/', {
            params: {
                ...params,
                replied: params.replied === undefined ? undefined : params.replied ? 'true' : 'false',
            },
        });
        return adaptPaginated(data);
    },
