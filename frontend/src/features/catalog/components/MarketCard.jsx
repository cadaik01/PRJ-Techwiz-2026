...
export function MarketCard({ market, highlighted = false, onHover, className, }) {
    ...
    return (<article className={cn('market-card', highlighted && 'is-highlighted', className)} onMouseEnter={() => onHover?.(market.id)} onMouseLeave={() => onHover?.(null)}>
      <Link to={`/markets/${market.id}`} className="market-card__media-link">
        <div className="market-card__media">
          <LazyImage src={market.image ?? ''} alt={market.name} className="market-card__image"/>
          {market.distance_km !== null ? (<Badge className="market-card__distance" variant="secondary">
      ...
      <div className="market-card__body">
        ...
        <div className="market-card__days">
          {market.operating_days.map((d) => (<Badge key={d} variant="outline">
