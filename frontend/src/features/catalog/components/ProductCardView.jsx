...
export function ProductCardView({ product, className, }) {
    ...
    return (<motion.article layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className={cn('product-card', className)}>
      <Link to={`/products/${product.id}`} className="product-card__media-link">
        <div className="product-card__media">
          {product.image ? (<LazyImage src={product.image} alt={product.name} className="product-card__image"/>) : (<div className="product-card__placeholder">No image</div>)}
      ...
      <div className="product-card__footer">
        <Button className="product-card__add-btn" size="sm" disabled={!canAdd} onClick={() => {
            addItem({
