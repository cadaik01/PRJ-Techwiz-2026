...
export function auditSubject(log: AuditLogItem): string {
  const d = log.details ?? {};
  const parts: string[] = [];
...
export function auditReason(log: AuditLogItem): string | null {
  return text(log.details ?? {}, 'reason');
}

export function auditOutcome(log: AuditLogItem): 'ok' | 'refused' | 'unknown' {
