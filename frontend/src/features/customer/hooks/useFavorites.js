...
function useOptimisticFavoriteToggle(kind, add, remove) {
    ...
    return useMutation({
        mutationFn: async (id) => {
            const current = queryClient.getQueryData(QUERY_KEYS.FAVORITE_IDS) ??
                emptyFavoriteIds();
            const listKey = kind === 'market'
                ? 'market_ids'
                : kind === 'farmer'
                    ? 'farmer_ids'
                    : 'product_ids';
...
export function useToggleFavoriteMarket() {
    return useOptimisticFavoriteToggle('market', customerApi.addFavoriteMarket, customerApi.removeFavoriteMarket);
}
export function useToggleFavoriteFarmer() {
    return useOptimisticFavoriteToggle('farmer', customerApi.addFavoriteFarmer, customerApi.removeFavoriteFarmer);
}
export function useToggleFavoriteProduct() {
    return useOptimisticFavoriteToggle('product', customerApi.addFavoriteProduct, customerApi.removeFavoriteProduct);
}
