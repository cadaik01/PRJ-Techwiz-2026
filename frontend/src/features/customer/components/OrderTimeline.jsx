...
function branchModifier(tone) {
    if (tone === 'danger')
        return 'order-timeline__branch--danger';
    if (tone === 'warning')
        return 'order-timeline__branch--warning';
    ...
    return (<div className="order-timeline">
      <ol className="order-timeline__steps">
        {MAIN_STEPS.map((step, index) => {
            ...
            return (<li key={step.key} className={cn('order-timeline__step', done && 'is-done', current && status === 'READY_FOR_PICKUP' && 'is-pulse')}>
              <div className="order-timeline__icon-wrap">
                {done ? (<Check className="order-timeline__icon is-done"/>) : (<Circle className="order-timeline__icon is-pending"/>)}
              </div>
              <p className="order-timeline__label">{step.label}</p>
            </li>);
        })}
      </ol>
      {branch ? (<div className={cn('order-timeline__branch', branchModifier(branch.tone))}>
