...
import { OPEN_ORDER_STATUSES } from '@/utils/helpers/domain';
    ...
    return (<article className="order-card">
      ...
      <div className="order-card__foot">
        <div>
          ...
          {isOpen ? (<Countdown className="order-card__countdown" targetIso={order.pickup_start_at} label="Until pickup"/>) : null}
        </div>
        <div className="order-card__actions">
          <Button asChild size="sm" variant="outline">
            <Link to={`/app/orders/${order.id}`}>Details</Link>
          </Button>
          {onReorder ? (<Button size="sm" variant="secondary" onClick={() => onReorder(order.id)}>
              Reorder
            </Button>) : null}
          {canReview ? (<Button asChild size="sm">
