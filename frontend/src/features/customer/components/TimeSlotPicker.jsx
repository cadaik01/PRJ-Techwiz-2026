...
export function TimeSlotPicker({ options, value, onChange, className, }) {
    return (<div className={cn('time-slot-picker', className)}>
      {options.map((option) => (<div key={option.market_id} className="time-slot-picker__market">
            ...
            {option.dates.map((dateOption) => (<div key={dateOption.date}>
                ...
                <div className="time-slot-picker__slots">
                  {dateOption.slots.map((slot) => {
                    const selected = value?.market_id === option.market_id &&
                    ...
                    return (<button key={slot.pickup_slot_id} type="button" disabled={!slot.is_bookable} className={cn('time-slot-picker__slot', selected && 'is-selected')} onClick={() => onChange({
