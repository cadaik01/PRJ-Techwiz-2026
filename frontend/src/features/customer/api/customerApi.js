...
import { adaptNotification, adaptNotificationPage } from '@/lib/adapters/notification.adapter';
...
export const customerApi = {
    getDashboard: async () => {
        ...
        if (data.recent_notifications) {
            data.recent_notifications = data.recent_notifications.map((item) => adaptNotification(item));
        }
        return data;
    },
    ...
    cancelOrder: async (id, reason, version) => {
        const { data } = await axiosClient.post(`/orders/${id}/cancel/`, { reason }, { ifMatch: String(version) });
        return data;
    },
