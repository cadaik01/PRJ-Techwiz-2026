...
export type DemoUser = Me & { password: string; role_id: number };
...
function stripPassword(user: DemoUser): Me {
  return {
    id: user.id,
    email: user.email,
    role: user.role,
...
function toBeMe(user: DemoUser) {
  return {
    ...
    farmer_status: user.farmer_status,
    farmer_profile:
      user.role === 'FARMER' && user.farmer_status
...
export function userFromAccess(access: string | null): Me | null {
...
export function getUserByAccess(authHeader: string | null): Me | null {
  if (!authHeader?.startsWith('Bearer ')) return null;
  return userFromAccess(authHeader.slice(7));
}

export function getDemoUserByAccess(authHeader: string | null): DemoUser | null {
  if (!authHeader?.startsWith('Bearer ')) return null;
...
export function revokeRefresh(refresh: string) {
...
export function revokeAccess(access: string) {
  const session = sessions.get(access);
  if (!session) return;
...
export function registerCustomerUser(input: {
  ...
  if (dynamicUsers[email]) return { error: 'EMAIL_EXISTS' as const };
...
export function findDemoByEmail(email: string) {
...
export function getCustomerProfile(userId: number) {
  const user = Object.values(dynamicUsers).find((u) => u.id === userId);
