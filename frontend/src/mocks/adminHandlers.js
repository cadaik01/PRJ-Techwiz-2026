...
import { adminAnnouncements, adminCategories, adminCustomers, adminFarmers, adminMarkets, auditLogs, moderationProducts, moderationReviews, pushAudit, pushFarmerHistory, setAdminAnnouncements, setAdminCategories, setAdminCustomers, setAdminFarmers, setAdminMarkets, setModerationProducts, setModerationReviews, toFarmerDetail, toFarmerSummary } from '@/mocks/adminData';
...
export const adminHandlers = [
    http.get('/api/admin/dashboard/', ({ request }) => {
        ...
        const orders_last_30_days = Array.from({ length: 30 }, (_, i) => {
            ...
            const count = customerOrders.filter((o) => o.created_at.slice(0, 10) === date).length;
        ...
        return HttpResponse.json(envelope({
            ...
            order_count_today: customerOrders.filter((o) => o.created_at.slice(0, 10) === today).length || 4,
            orders_last_30_days,
    ...
    http.get('/api/admin/farmers/:id/impact/', ({ params, request }) => {
        ...
        const open = customerOrders.filter((o) => o.farmer.id === farmer.id &&
            (o.status === 'PLACED' ||
                o.status === 'ACCEPTED' ||
    ...
    http.post('/api/admin/farmers/:id/reject/', async ({ params, request }) => {
        ...
        if (reason.length < 5) {
            return HttpResponse.json(errorEnvelope('Reason must be at least 5 characters', 'VALIDATION_ERROR'), { status: 400 });
        }
    ...
    http.get('/api/admin/customers/', ({ request }) => {
        ...
        if (q) {
            list = list.filter((c) => c.email.toLowerCase().includes(q) || c.full_name.toLowerCase().includes(q));
        }
    ...
    http.post('/api/admin/customers/:id/deactivate/', async ({ params, request }) => {
        ...
        return HttpResponse.json(envelope({ ...next, affected_orders }, 'Account deactivated'));
    }),
    ...
    http.delete('/api/admin/categories/:id/', ({ params, request }) => {
        ...
        if (cat.product_count > 0) {
            return HttpResponse.json(errorEnvelope('Category is in use', 'RESOURCE_IN_USE'), { status: 400 });
        }
    ...
    http.get('/api/admin/reports/', ({ request }) => {
        ...
        if (from)
            orders = orders.filter((o) => o.created_at.slice(0, 10) >= from);
        if (to)
            orders = orders.filter((o) => o.created_at.slice(0, 10) <= to);
        if (marketId)
            orders = orders.filter((o) => o.market.id === Number(marketId));
        const statusMap = new Map();
    ...
    http.get('/api/admin/reports/export/', ({ request }) => {
        ...
        const rows = [
            ...
            ...customerOrders
                ...
                .map((o) => `${String(o.id)},${o.status},${o.farmer.stall_name},${o.market.name},${o.total_amount},${o.created_at}`),
        ];
    ...
    http.get('/api/admin/audit-logs/', ({ request }) => {
        ...
        if (user)
            list = list.filter((l) => (l.user?.email ?? '').includes(user));
        if (from)
            list = list.filter((l) => l.created_at.slice(0, 10) >= from);
        if (to)
            list = list.filter((l) => l.created_at.slice(0, 10) <= to);
