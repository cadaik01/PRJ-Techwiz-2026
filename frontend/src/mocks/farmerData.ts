...
export let farmerProducts: FarmerProduct[] = products
...
export function setFarmerProducts(next: FarmerProduct[]) {
  farmerProducts = next;
}

export function bumpProductVersion(product: FarmerProduct): FarmerProduct {
  return {
    ...product,
...
function slotsFromPickup(option: (typeof pickupOptions)[number][number]): PickupSlot[] {
...
function buildMemberships(): FarmerMarketMembership[] {
...
export let farmerMarkets: FarmerMarketMembership[] = buildMemberships();

export function setFarmerMarkets(next: FarmerMarketMembership[]) {
  farmerMarkets = next;
}
...
export function setFarmerProfile(next: FarmerProfile) {
  farmerProfile = next;
}

export const farmerReviews: FarmerReviewItem[] = reviews.filter((r) => {
...
export function replyFarmerReview(id: number, reply: string): FarmerReviewItem | null {
  const idx = farmerReviews.findIndex((r) => r.id === id);
  if (idx < 0) return null;
  const current = farmerReviews[idx];
  if (current.reply) return null;
  const updated: FarmerReviewItem = {
    ...current,
    reply,
...
export const farmerNotifications: NotificationItem[] = [
  {
    id: 101,
    type: 'ORDER_PLACED',
    title: 'Đơn mới #6',
