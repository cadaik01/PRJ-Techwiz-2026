import { announcements, categories, farmers, markets, pickupOptions, products, reviews, toFarmerSummary, toProductCard } from '@/mocks/catalog';
...
function ratingSummary(list) {
    ...
    const rating_avg = rating_count === 0 ? null : list.reduce((s, r) => s + r.rating, 0) / rating_count;
...
export const catalogHandlers = [
    ...
    http.get('/api/farmers/', ({ request }) => {
        ...
        let list = farmers.map((f) => {
            ...
            return {
                ...summary,
                distance_km: hasCoords && f.latitude != null && f.longitude != null
        ...
        if (categoryId) {
            ...
            list = list.filter((f) => products.some((p) => p.farmer.id === f.id && p.category.id === cat));
        }
        if (ordering === 'rating') {
            list = [...list].sort((a, b) => (b.rating_avg ?? 0) - (a.rating_avg ?? 0));
        }
        else if (ordering === 'in_stock') {
            list = [...list].sort((a, b) => b.in_stock_product_count - a.in_stock_product_count);
        }
    ...
    http.get('/api/farmers/:id/reviews/', ({ params, request }) => {
        ...
        const list = reviews.filter((r) => (r.type === 'FARMER' && id === 2 && r.id === 1) ||
            (r.type === 'PRODUCT' &&
                r.product != null &&
    ...
    http.get('/api/products/', ({ request }) => {
        ...
        if (marketId) {
            list = list.filter((p) => products
                .find((full) => full.id === p.id)
                ?.markets.some((m) => m.market_id === Number(marketId)));
        }
        if (farmerId)
            list = list.filter((p) => p.farmer.id === Number(farmerId));
    ...
    http.get('/api/products/:id/', ({ params, request }) => {
        const user = getUserByAccess(request.headers.get('Authorization'));
        if (user?.role === 'FARMER') {
            return forwardFarmer(request, `/api/__farmer_only/products/${numParam(params.id)}/`);
        }
        const id = numParam(params.id);
