import { farmers, markets, pickupOptions, products, toFarmerSummary, toProductCard } from '@/mocks/catalog';
import { envelope, errorEnvelope, getCustomerProfile, getUserByAccess, paginate } from '@/mocks/data';
import { farmerNotifications } from '@/mocks/farmerData';
import { numParam, pageParams } from '@/mocks/mockUtils';
import { bumpOrderVersion, customerNotifications, customerOrders, isOpenStatus, setOrders, toOrderSummary } from '@/mocks/orders';
...
export const customerHandlers = [
    http.get('/api/dashboard/', ({ request }) => {
        ...
        return HttpResponse.json(envelope({
            counts: {
                ...
                pending_review: completed.filter((o) => o.review_state != null &&
                    (!o.review_state.farmer_reviewed ||
                        o.review_state.items_pending_review.length > 0)).length,
            },
    ...
    http.get('/api/orders/', ({ request }) => {
        ...
        if (tab === 'open')
            list = list.filter((o) => isOpenStatus(o.status));
        if (tab === 'history')
            list = list.filter((o) => !isOpenStatus(o.status));
        const farmerId = url.searchParams.get('farmer_id');
        if (farmerId)
            list = list.filter((o) => o.farmer.id === Number(farmerId));
        return HttpResponse.json(envelope(paginate(list.map(toOrderSummary), page, page_size)));
    }),
        ...
        for (const group of body.groups) {
            ...
            for (const opt of options) {
                for (const dateOpt of opt.dates) {
                    if (dateOpt.date !== group.pickup_date)
                        continue;
                    const slot = dateOpt.slots.find((s) => s.pickup_slot_id === group.pickup_slot_id);
                    if (slot) {
                        slotMeta = {
    ...
    http.patch('/api/orders/:id/', async ({ params, request }) => {
        ...
        if (body.pickup_slot_id !== undefined)
            next.pickup_slot_id = body.pickup_slot_id;
        if (body.pickup_date !== undefined)
            next.pickup_date = body.pickup_date;
        ...
        for (const item of order.items) {
            ...
            if (!product || product.availability !== 'IN_STOCK') {
                skipped.push({
                    ...
                    reason: product && !product.is_available
                        ? 'UNAVAILABLE'
                        : 'OUT_OF_STOCK',
                });
            }
    ...
    http.post('/api/orders/:orderId/items/:itemId/review/', async ({ params, request }) => {
        ...
        const pending = (order.review_state?.items_pending_review ?? []).filter((pid) => pid !== item?.product_id);
        const review_state = {
            farmer_reviewed: order.review_state?.farmer_reviewed ?? false,
            items_pending_review: pending,
        };
        const done = review_state.farmer_reviewed && review_state.items_pending_review.length === 0;
        const next = bumpOrderVersion({
            ...order,
            review_state,
