...
const favoriteIds = {
  farmer_ids: [2] as number[],
  product_ids: [2, 6] as number[],
  market_ids: [1] as number[],
};
...
function ifMatchVersion(request: Request, order: OrderDetail) {
...
export const customerHandlers = [
  ...
  http.post('/api/orders/', async ({ request }) => {
    ...
    const body = (await request.json()) as CreateOrdersPayload;
  ...
  http.patch('/api/orders/:id/', async ({ params, request }) => {
    ...
    if (body.items) {
      const items: OrderItem[] = body.items.map((line) => {
        ...
        const unit: Unit = existing?.unit ?? product?.unit ?? 'KG';
        return {
  ...
  http.post('/api/favorite-farmers/', async ({ request }) => {
    ...
    const body = (await request.json()) as { farmer_id: number };
  ...
  http.post('/api/favorite-markets/', async ({ request }) => {
    ...
    const body = (await request.json()) as { market_id: number };
