import { allMarkets, bumpProductEtag, categoryName, createEmptySlot, DEMO_FARMER_ID, farmerMarkets, farmerNotifications, farmerProducts, farmerProfile, farmerReviews, replyFarmerReview, setFarmerMarkets, setFarmerProducts, setFarmerProfile } from '@/mocks/farmerData';
import { envelope, errorEnvelope, getUserByAccess, paginate } from '@/mocks/data';
import { bumpOrderVersion, customerOrders, isOpenStatus, setOrders, toOrderSummary } from '@/mocks/orders';
...
function computeActions(order, now = Date.now()) {
    ...
    if (order.status === 'READY_FOR_PICKUP' && pickupEnded) {
        ...
        if (!available.includes('NO_SHOW'))
            available.push('NO_SHOW');
    }
    return available;
}
...
function checkIfMatch(request, version) {
    ...
    if (match && Number(match) !== version) {
        return HttpResponse.json(errorEnvelope('Resource was modified', 'RESOURCE_MODIFIED'), { status: 409 });
    }
    return null;
}
function productStateFilter(p, state) {
    ...
    if (state === 'archived')
        return p.is_archived;
    if (state === 'hidden')
        return p.is_hidden_by_admin;
    if (state === 'in_stock')
        return p.availability === 'IN_STOCK' && !p.is_archived;
    if (state === 'out_of_stock')
        return p.availability === 'OUT_OF_STOCK' && !p.is_archived;
    if (state === 'unavailable')
        return p.availability === 'UNAVAILABLE' && !p.is_archived;
    return true;
}
export const farmerHandlers = [
    http.get('/api/__farmer_only/dashboard/', ({ request }) => {
        ...
        const processing = orders.filter((o) => o.status === 'ACCEPTED' || o.status === 'READY_FOR_PICKUP').length;
    ...
    http.get('/api/__farmer_only/orders/', ({ request }) => {
        ...
        if (tab === 'pending')
            ...
            list = list.filter((o) => o.status === 'COMPLETED' ||
                ...
                o.status === 'NO_SHOW');
        if (marketId)
            list = list.filter((o) => o.market.id === Number(marketId));
        if (pickupDate)
            list = list.filter((o) => o.pickup_date === pickupDate);
        if (q) {
            list = list.filter((o) => {
        ...
        return HttpResponse.json(envelope(paginate(list.map(toFarmerSummary), page, page_size)));
    }),
    ...
    http.post('/api/orders/:id/accept/', ({ params, request }) => {
        ...
        if (conflict)
            return conflict;
        if (order.status !== 'PLACED') {
            return HttpResponse.json(errorEnvelope('Invalid status transition', 'INVALID_STATUS_TRANSITION'), { status: 400 });
        }
    ...
    http.post('/api/orders/:id/decline/', async ({ params, request }) => {
        ...
        if (reason.length < 5 || reason.length > 500) {
            return HttpResponse.json(errorEnvelope('Rejection reason must be 5–500 characters', 'VALIDATION_ERROR'), { status: 400 });
        }
        if (order.status !== 'PLACED') {
            return HttpResponse.json(errorEnvelope('Invalid status transition', 'INVALID_STATUS_TRANSITION'), { status: 400 });
        }
    ...
    http.post('/api/orders/:id/complete/', ({ params, request }) => {
        ...
        if (Date.now() < new Date(order.pickup_end_at).getTime()) {
            return HttpResponse.json(errorEnvelope('Pickup window has not ended', 'PICKUP_NOT_ENDED'), { status: 400 });
        }
        const next = bumpOrderVersion({
            ...order,
            status: 'COMPLETED',
    ...
    http.get('/api/picking-list/', ({ request }) => {
        ...
        const url = new URL(request.url);
        const pickupDate = url.searchParams.get('pickup_date') ?? new Date().toISOString().slice(0, 10);
        const map = new Map();
        for (const order of farmerOrderList(user.id)) {
            if (order.pickup_date !== pickupDate)
                continue;
            if (order.status !== 'ACCEPTED' &&
    ...
    http.post('/api/products/', async ({ request }) => {
        ...
        if (user.farmer_status !== 'APPROVED') {
            return HttpResponse.json(errorEnvelope('Profile not approved', 'FARMER_NOT_APPROVED'), { status: 403 });
        }
        const body = (await request.json());
    ...
    http.patch('/api/products/:id/', async ({ params, request }) => {
        ...
        let next = {
            ...
            stock_quantity,
            is_available,
            availability: productAvailability(stock_quantity, is_available, current.is_hidden_by_admin, current.is_archived),
            ...
            weekly_default_quantity: body.weekly_default_quantity !== undefined
    ...
    http.post('/api/weekly-template/apply/', ({ request }) => {
        ...
        if (overdue.length > 0) {
            return HttpResponse.json(errorEnvelope('Overdue orders still pending', 'OVERDUE_ORDERS_PENDING'), { status: 400 });
        }
    ...
    http.get('/api/reviews/', ({ request }) => {
        ...
        if (replied === 'true')
            list = list.filter((r) => Boolean(r.reply));
        if (replied === 'false')
    ...
    http.post('/api/reviews/:id/reply/', async ({ params, request }) => {
        ...
        if (reply.length < 1 || reply.length > 500) {
            return HttpResponse.json(errorEnvelope('Reply must be 1–500 characters', 'VALIDATION_ERROR'), { status: 400 });
        }
        ...
        if (existing.reply) {
            return HttpResponse.json(errorEnvelope('Already replied to this review', 'REPLY_ALREADY_EXISTS'), { status: 400 });
        }
