...
export const Input = React.forwardRef                              (
  ({ className, type, id, label, placeholder, disabled, requiredMark, ...props }, ref) => {
    ...
    const toggleButton = isPassword ? (
      ...
      >
        {passwordVisible ? (
          <EyeOff className="input-field__toggle-icon" strokeWidth={1.75} aria-hidden />
        ) : (
          <Eye className="input-field__toggle-icon" strokeWidth={1.75} aria-hidden />
        )}
      </button>
    ) : null;
    ...
    if (!useFloating) {
      ...
      return (
        <div className={cn('input-field', 'input-field--password', className)}>
          <input
            id={id}
            type={inputType}
    ...
    return (
      <div className={cn('input-field', isPassword && 'input-field--password', className)}>
        <input
          id={inputId}
          type={inputType}
          className={cn('input', isPassword && 'input--password')}
          placeholder=" "
          disabled={disabled}
          ref={ref}
          {...props}
