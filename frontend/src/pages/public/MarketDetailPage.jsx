...
export default function MarketDetailPage() {
    ...
    if (marketQuery.isError || !marketQuery.data) {
        return (<div className="market-detail-page__empty-wrap">
        <EmptyState title="This market could not be found" actionLabel="Try again" onAction={() => marketQuery.refetch()}/>
      </div>);
    }
    const market = marketQuery.data;
    const favorited = hasMarket(market.id);
    const addressLine = market.address;
    return (<div className="market-detail-page">
      <section className="market-detail-page__banner">
        ...
        <div className="market-detail-page__banner-inner">
          ...
          <div className="market-detail-page__banner-head">
            ...
            <FavoriteButton active={favorited} className="market-detail-page__fav-btn" onToggle={() => {
      ...
      <div className="market-detail-page__body">
        <div className="market-detail-page__meta-row">
          ...
          {market.distance_km !== null ? (<MetaChip icon={<Navigation strokeWidth={1.75} aria-hidden/>} label={`${market.distance_km.toFixed(1)} km`}/>) : null}
          <MetaChip icon={<Store strokeWidth={1.75} aria-hidden/>} label={`${market.farmer_count} stalls`}/>
        </div>
        ...
        <div className="market-detail-page__split">
          <div className="market-detail-page__about">
            ...
            <div>
              ...
              <div className="market-detail-page__days">
                {market.operating_days.map((d) => (<Badge key={d} variant="secondary" className="market-detail-page__day-badge">
                    {WEEKDAY_LABELS[d]}
                  </Badge>))}
              </div>
            </div>
          ...
          <div className="market-detail-page__map-col">
            ...
            <Suspense fallback={<Skeleton className="market-detail-page__map-skeleton"/>}>
              <MiniMap latitude={market.latitude} longitude={market.longitude} label={market.name} className="market-detail-page__map"/>
            </Suspense>
          </div>
        </div>
        ...
        <section className="market-detail-page__farmers">
          <div className="market-detail-page__farmers-head">
            ...
            {farmersQuery.data ? (<p className="market-detail-page__farmers-count">
                {farmersQuery.data.length} stalls
              </p>) : null}
          </div>

          {farmersQuery.isLoading ? (<div className="market-detail-page__farmers-grid">
              {Array.from({ length: 4 }).map((_, i) => (<Skeleton key={i} className="market-detail-page__farmer-skeleton"/>))}
            </div>) : farmersQuery.isError ? (<EmptyState title="Stalls couldn't be loaded" actionLabel="Try again" onAction={() => farmersQuery.refetch()}/>) : !farmersQuery.data?.length ? (<EmptyState title="No stalls are listed at this market yet"/>) : (<div className="market-detail-page__farmers-grid">
              {farmersQuery.data.map((farmer) => (<div key={farmer.id} className="market-detail-page__farmer-item">
...
function MetaChip({ icon, label }) {
    return (<span className="market-detail-page__meta-chip">
