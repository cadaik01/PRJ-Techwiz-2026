...
import { useProduct, useProductRating, useProductReviews, useProducts } from '@/features/catalog/hooks/useCatalog';
...
function ratingFillClass(pct) {
    if (pct === 0)
        return 'product-detail-page__rating-fill--0';
    if (pct <= 20)
        return 'product-detail-page__rating-fill--1';
    if (pct <= 40)
        return 'product-detail-page__rating-fill--2';
    if (pct <= 60)
        return 'product-detail-page__rating-fill--3';
    if (pct <= 80)
        return 'product-detail-page__rating-fill--4';
...
export default function ProductDetailPage() {
    ...
    const related = useMemo(() => relatedQuery.data?.results.filter((p) => String(p.id) !== id).slice(0, 4) ?? [], [relatedQuery.data, id]);
    if (productQuery.isLoading)
        return <PageSkeleton />;
    if (productQuery.isError || !productQuery.data) {
        return (<div className="product-detail-page__empty-wrap">
        <EmptyState title="This product is unavailable" actionLabel="Try again" onAction={() => productQuery.refetch()}/>
      </div>);
    }
    const product = productQuery.data;
    const favorited = hasProduct(product.id);
    const canAdd = product.availability === 'IN_STOCK' && product.stock_quantity > 0;
    const stockLabel = product.availability === 'UNAVAILABLE'
        ? 'Unavailable'
    ...
    return (<div className="product-detail-page">
      <div className="product-detail-page__main">
        <div>
          <div className="product-detail-page__media">
            {product.image ? (<LazyImage src={product.image} alt={product.name} className="product-detail-page__media-img"/>) : (<div className="product-detail-page__media-placeholder">Photo coming soon</div>)}
          </div>
        </div>
        ...
        <div className="product-detail-page__info">
          ...
          <PriceTag amount={product.price} unit={product.unit} className="product-detail-page__price"/>
          <Badge variant={product.availability === 'IN_STOCK'
            ? product.stock_quantity <= 10
                ? 'warning'
                : 'success'
            : 'danger'}>
          ...
          <div className="product-detail-page__stall-box">
            ...
            {product.markets[0] ? (<p className="product-detail-page__market-note">
                At {product.markets[0].market_name}
              </p>) : null}
          </div>

          <div className="product-detail-page__actions">
            <QuantityStepper value={qty} max={Math.max(1, product.stock_quantity)} onChange={setQty} disabled={!canAdd}/>
      ...
      <div className="product-detail-page__reviews-layout">
        <Card className="product-detail-page__rating-card">
          ...
          <CardContent className="product-detail-page__card-content--stack">
            ...
            ? ['5', '4', '3', '2', '1'].map((star) => {
                ...
                const pct = summary.rating_count === 0
                    ? 0
                    : Math.round((count / summary.rating_count) * 100);
                return (<div key={star} className="product-detail-page__rating-row">
                      <span className="product-detail-page__rating-star-label">
                        {star}★
                      </span>
                      <div className="product-detail-page__rating-track">
                        <div className={`product-detail-page__rating-fill ${ratingFillClass(pct)}`}/>
                      </div>
        ...
        <Card className="product-detail-page__comments-card">
          ...
          <CardContent className="product-detail-page__comments-stack">
            {reviewsQuery.data?.results.length ? (reviewsQuery.data.results.map((review) => (<div key={review.id} className="product-detail-page__comment">
                  ...
                  {review.reply ? (<div className="product-detail-page__reply">
                ...
                </div>))) : (<p className="product-detail-page__no-comments">
      ...
      {related.length > 0 ? (<section className="product-detail-page__related">
