...
function parseMarketSort(value) {
    if (value === 'distance' || value === 'name' || value === 'name_desc')
        return value;
    return 'name';
}
const DAY_OPTIONS = [
...
export default function MarketsPage() {
    ...
    return (<div className="markets-page">
      ...
      <div className="markets-page__body">
        <div className="markets-page__toolbar">
          <div className="markets-page__search-row">
            <div className="markets-page__search-wrap">
              <Input value={q} onChange={(e) => setQ(e.target.value)} label="Search markets or addresses" className="markets-page__search-input"/>
            </div>

            <select className="markets-page__select" value={sort} onChange={(e) => setSort(parseMarketSort(e.target.value))} aria-label="Sort markets">
          ...
          <div className="markets-page__day-list">
            ...
            return (<button key={d.value} type="button" className={active
                    ? 'markets-page__day-btn markets-page__day-btn--active'
                    : 'markets-page__day-btn'} onClick={() => setWeekday(active ? undefined : d.value)}>
                  {d.label}
                </button>);
        })}
          </div>
        ...
        {!query.isLoading && !query.isError && markets.length > 0 ? (<p className="markets-page__count">{markets.length} markets</p>) : null}

        {query.isLoading ? (<div className="markets-page__grid">
            {Array.from({ length: 6 }).map((_, i) => (<Skeleton key={i} className="markets-page__skeleton"/>))}
          </div>) : query.isError ? (<EmptyState title="Markets couldn't be loaded" actionLabel="Try again" onAction={() => query.refetch()}/>) : markets.length === 0 ? (<EmptyState title="No markets match your filters" description="Try another day, keyword, or clear your search."/>) : (<div className="markets-page__grid">
