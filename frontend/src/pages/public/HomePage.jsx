...
import { Apple, ArrowRight, Egg, Flame, Leaf, LocateFixed, Milk, Wheat } from 'lucide-react';
...
import { useCategories, useFarmers, useMarkets, useProducts } from '@/features/catalog/hooks/useCatalog';
...
function SectionHeading({ title, description, href, linkLabel = 'View all', }) {
    return (<div className="home-page__section-heading">
...
export default function HomePage() {
    ...
    return (<div className="home-page">
      <section className="home-page__hero">
        ...
        <div className="home-page__hero-inner">
          <div className="home-page__hero-copy">
            ...
            <form className="home-page__search" onSubmit={(e) => {
            e.preventDefault();
            const term = q.trim();
            if (!term) {
                navigate('/products');
            ...
            navigate(`/products?q=${encodeURIComponent(term)}`);
              ...
              <Input id="home-search" value={q} onChange={(e) => setQ(e.target.value)} label="Search produce, stalls, or markets" className="home-page__search-input"/>
      ...
      <section className="home-page__categories">
        <div className="home-page__categories-inner">
          ...
          <div className="home-page__categories-scroll">
            ...
            ? Array.from({ length: 6 }).map((_, i) => (<Skeleton key={i} className="home-page__category-skeleton"/>))
            : categoriesQuery.data?.map((cat) => {
                const Icon = resolveCategoryIcon(cat.icon);
                return (<Link key={cat.id} to={`/products?category=${cat.id}`} className="home-page__category-card">
      ...
      <section className="home-page__section">
        <SectionHeading title="Markets near you" description={lat != null
            ? 'Closest markets to where you are right now'
            : 'Share your location to see the nearest markets first'} href="/markets" linkLabel="See all markets"/>
        {marketsQuery.isLoading ? (<div className="home-page__grid-markets">
            {Array.from({ length: 3 }).map((_, i) => (<Skeleton key={i} className="home-page__skeleton-market"/>))}
          </div>) : marketsQuery.isError ? (<EmptyState title="Markets couldn't be loaded" actionLabel="Try again" onAction={() => marketsQuery.refetch()}/>) : (<div className="home-page__grid-markets">
            {marketsQuery.data?.results.slice(0, 3).map((m) => (<MarketCard key={m.id} market={m}/>))}
          </div>)}
      </section>

      <section className="home-page__section--highlight">
        <div className="home-page__section-inner">
          <SectionHeading title="Today's standouts" description="Top-rated produce, ready to pre-order and pick up" href="/products" linkLabel="Browse all produce"/>
          {productsQuery.isLoading ? (<div className="home-page__grid-products">
              {Array.from({ length: 4 }).map((_, i) => (<Skeleton key={i} className="home-page__skeleton-product"/>))}
            </div>) : productsQuery.isError ? (<EmptyState title="Products couldn't be loaded" actionLabel="Try again" onAction={() => productsQuery.refetch()}/>) : (<div className="home-page__grid-products">
              {productsQuery.data?.results.slice(0, 8).map((p) => (<ProductCardView key={p.id} product={p}/>))}
            </div>)}
        </div>
      </section>

      <section className="home-page__section">
        <SectionHeading title="Stalls worth knowing" description="Meet highly rated farmers before you place your order" href="/farmers" linkLabel="Browse all stalls"/>
        {farmersQuery.isLoading ? (<div className="home-page__grid-farmers">
            {Array.from({ length: 4 }).map((_, i) => (<Skeleton key={i} className="home-page__skeleton-farmer"/>))}
          </div>) : farmersQuery.isError ? (<EmptyState title="Stalls couldn't be loaded" actionLabel="Try again" onAction={() => farmersQuery.refetch()}/>) : (<div className="home-page__grid-farmers">
            {farmersQuery.data?.results.slice(0, 4).map((f) => (<FarmerCard key={f.id} farmer={f}/>))}
          </div>)}
      </section>
      ...
      <section className="home-page__how">
        <div className="home-page__how-inner">
          ...
          <ol className="home-page__steps">
            {STEPS.map((step, index) => (<li key={step.title} className="home-page__step">
