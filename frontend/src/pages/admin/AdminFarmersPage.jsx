...
import { fetchFarmerImpact, useAdminFarmers, useApproveFarmer, useReinstateFarmer, useRejectFarmer, useSuspendFarmer } from '@/features/admin/hooks/useAdminFarmers';
...
export default function AdminFarmersPage() {
    ...
    const openSuspend = async (id) => {
        try {
            ...
            setImpactText(`Impact: ${impact.open_order_count} open orders, ${impact.affected_customer_count} customers.`);
            setSuspendId(id);
            setReason('');
        }
        catch (e) {
    ...
    return (<div className="admin-farmers-page">
      <PageHeader eyebrow="Grower accounts" title="Farmer stalls" description="Approve new growers, suspend accounts, and restore access."/>

      <div className="page-primitive__toolbar">
        <Input label="Search stall / email" value={q} onChange={(e) => {
        ...
        <select className="page-primitive__select" value={status ?? ''} onChange={(e) => {
            const next = new URLSearchParams(params);
            if (e.target.value)
      ...
      {query.isLoading ? (<PageSkeleton />) : query.isError ? (<EmptyState title="Stalls couldn't be loaded" actionLabel="Try again" onAction={() => query.refetch()}/>) : !query.data?.results.length ? (<EmptyState title="No farmer stalls yet"/>) : (<>
          <div className="page-primitive__table-wrap">
            <table className="page-primitive__table page-primitive__table-min-800">
              ...
              <tbody>
                {query.data.results.map((f) => (<tr key={f.id} className="page-primitive__table-row">
                    ...
                    <td className="page-primitive__table-td">
                      <div className="page-primitive__actions-row">
                        ...
                        {f.status === 'APPROVED' ? (<Button size="sm" variant="destructive" onClick={() => openSuspend(f.id)}>
                            Suspend
                          </Button>) : null}
                        {f.status === 'SUSPENDED' || f.status === 'REJECTED' ? (<Button size="sm" onClick={() => reinstate.mutate(f.id)}>
                            Restore
                          </Button>) : null}
                      </div>
                    </td>
      ...
      <ConfirmDialog open={Boolean(rejectId)} onOpenChange={(open) => {
            if (!open)
                setRejectId(null);
        }} title="Decline stall application" description="Share a clear reason (at least 5 characters)." destructive loading={reject.isPending} onConfirm={() => {
            if (!rejectId || reason.trim().length < 5) {
                toast.error('Reason must be at least 5 characters');
                return;
            }
            reject.mutate({ id: rejectId, reason }, {
                onSuccess: () => {
                    setRejectId(null);
                    setReason('');
                },
      ...
      <ConfirmDialog open={Boolean(suspendId)} onOpenChange={(open) => {
            if (!open)
                setSuspendId(null);
        }} title="Suspend this stall" description={impactText} destructive loading={suspend.isPending} onConfirm={() => {
            if (!suspendId || reason.trim().length < 5) {
                toast.error('Reason must be at least 5 characters');
                return;
            }
            suspend.mutate({ id: suspendId, reason }, {
                onSuccess: () => {
                    setSuspendId(null);
                    setReason('');
                },
