...
export default function AdminAuditLogsPage() {
    ...
    return (<div className="admin-audit-logs-page">
      <PageHeader eyebrow="Activity log" title="Audit trail" description="A chronological log of admin actions across the platform."/>

      <div className="page-primitive__toolbar">
        <Input label="Action (e.g. FARMER_APPROVE)" value={action} onChange={(e) => {
            setAction(e.target.value);
            setFilters((prev) => ({ ...prev, action: e.target.value }));
        }} className="page-primitive__input-narrow"/>
        <Input label="Email actor" value={user} onChange={(e) => {
            setUser(e.target.value);
            setFilters((prev) => ({ ...prev, user: e.target.value }));
        }} className="page-primitive__input-narrow"/>
        <Input type="date" label="From" value={from} onChange={(e) => {
            setFrom(e.target.value);
            setFilters((prev) => ({ ...prev, from: e.target.value }));
        }} className="page-primitive__input-auto"/>
        <Input type="date" label="To" value={to} onChange={(e) => {
            setTo(e.target.value);
            setFilters((prev) => ({ ...prev, to: e.target.value }));
        }} className="page-primitive__input-auto"/>
      </div>

      {query.isLoading ? (<PageSkeleton />) : !query.data?.results.length ? (<EmptyState title="No audit entries yet"/>) : (<div className="page-primitive__table-wrap">
          <table className="page-primitive__table page-primitive__table-min-720">
            ...
            <tbody>
              {query.data.results.map((log) => (<tr key={log.id} className="page-primitive__table-row">
      ...
      <Sheet open={Boolean(selected)} onOpenChange={(open) => {
        ...
        <SheetContent className="page-primitive__sheet-md">
          ...
          {selected ? (<pre className="page-primitive__pre-box">
