...
import { useAdminMarket, useSaveAdminMarket } from '@/features/admin/hooks/useAdminMarkets';
import { marketSchema } from '@/features/admin/schemas/market.schema';
...
function readDragLatLng(target) {
    if (!target || typeof target !== 'object' || !('getLatLng' in target))
        return null;
...
export default function AdminMarketFormPage() {
    ...
    useEffect(() => {
        if (!isEdit || !market || seededId === market.id)
            return;
        form.reset({
            name: market.name,
    ...
    if (isEdit && marketQuery.isLoading)
        return <PageSkeleton />;
    ...
    return (<form className="admin-market-form-page" onSubmit={form.handleSubmit(onSubmit)}>
      <PageHeader eyebrow="Market setup" title={isEdit ? 'Edit market' : 'Add a market'}/>
        ...
        <div className="admin-market-form-page__fields">
          <div className="page-primitive__form-grid-2">
            ...
            <div className="page-primitive__form-field">
              <Input id="open_time" type="time" label="Open time" {...form.register('open_time')}/>
              {form.formState.errors.open_time ? (<p className="page-primitive__error">
                  {form.formState.errors.open_time.message}
                </p>) : null}
            </div>
            <div className="page-primitive__form-field">
              <Input id="close_time" type="time" label="Close time" {...form.register('close_time')}/>
              {form.formState.errors.close_time ? (<p className="page-primitive__error">
          ...
          <div>
            ...
            <div className="admin-market-form-page__days">
              {DAYS.map((day) => (<button key={day.value} type="button" onClick={() => toggleDay(day.value)} className={operatingDays.includes(day.value)
                ? 'admin-market-form-page__day admin-market-form-page__day--active'
                : 'admin-market-form-page__day'}>
                  {day.label}
                </button>))}
            </div>
            {form.formState.errors.operating_days ? (<p className="page-primitive__error page-primitive__mt-2">
        ...
        <div className="admin-market-form-page__map">
          <div className="page-primitive__map-box">
            <MapContainer center={[latitude, longitude]} zoom={15} className="page-primitive__map-fill">
              ...
              <Marker position={[latitude, longitude]} draggable eventHandlers={{
            dragend: (event) => {
              ...
              <MapClick onPick={(nextLatitude, nextLongitude) => {
