...
function isSuccessState(value) {
    if (!value || typeof value !== 'object')
        return false;
    if (!('orders' in value) || !Array.isArray(value.orders))
        return false;
...
function confettiColorClass(index) {
    const mod = index % 3;
    if (mod === 0)
        return 'order-success-page__confetti--primary';
    if (mod === 1)
        return 'order-success-page__confetti--accent';
...
export default function OrderSuccessPage() {
    ...
    return (<div className="order-success-page">
      <div className="order-success-page__confetti-layer" aria-hidden>
        {pieces.map((p) => (<motion.span key={p.id} className={`order-success-page__confetti ${p.posClass} ${p.colorClass}`} initial={{ y: -20, opacity: 1 }} animate={{ y: 420, opacity: 0, rotate: 180 }} transition={{ duration: 1.8, delay: p.delay, ease: 'easeOut' }}/>))}
      </div>
      ...
      <div className="order-success-page__hero page-primitive__max-w-2xl page-primitive__center-col">
        ...
        <PageHeader className="order-success-page__header" eyebrow="You're all set" title="Pre-order confirmed" description={`${orders.length} order${orders.length === 1 ? '' : 's'} ready. Pay when you collect at the stall.`}/>
      </div>

      <div className="order-success-page__orders page-primitive__max-w-2xl">
        {orders.map((order) => (<div key={order.id} className="order-success-page__order-card">
