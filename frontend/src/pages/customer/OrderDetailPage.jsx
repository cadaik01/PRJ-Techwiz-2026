...
import { useCancelOrder, useCustomerOrder } from '@/features/customer/hooks/useCustomerOrders';
...
export default function OrderDetailPage() {
    ...
    if (orderQuery.isError || !orderQuery.data) {
        return (<EmptyState title="This order could not be found" actionLabel="Try again" onAction={() => orderQuery.refetch()}/>);
    }
    ...
    return (<div className="order-detail-page">
      <PageHeader eyebrow="Order details" title={orderLabel} description={`${order.farmer.stall_name} · ${order.market.name}`} actions={<StatusBadge status={order.status}/>}/>
      ...
      <div className="order-detail-page__grid">
        <div className="order-detail-page__panel">
          ...
          {order.stall_label ? (<p className="order-detail-page__stall">{order.stall_label}</p>) : null}
          <p className="order-detail-page__pickup-time">
            {formatDateTime(order.pickup_start_at)}
          </p>
          <Countdown className="order-detail-page__countdown" targetIso={order.cutoff_at} label="Cut-off"/>
          <Countdown className="order-detail-page__countdown order-detail-page__countdown--tight" targetIso={order.pickup_start_at} label="Pickup time"/>
          <Button asChild variant="outline" size="sm" className="order-detail-page__directions">
            <a href={googleMapsDirectionsUrl(order.market.latitude, order.market.longitude)} target="_blank" rel="noreferrer">
      ...
      <div className="order-detail-page__panel">
        ...
        <ul className="order-detail-page__items">
          {order.items.map((item) => (<li key={item.id} className="order-detail-page__item-row">
              <span>
        ...
        {order.note ? (<p className="order-detail-page__note">Note: {order.note}</p>) : null}
      </div>

      <div className="order-detail-page__actions">
        {canEdit ? (<Button asChild variant="secondary">
            <Link to={`/app/orders/${order.id}/edit`}>Edit order</Link>
          </Button>) : null}
        {canCancel ? (<Button variant="destructive" onClick={() => setCancelOpen(true)}>
            Cancel order
          </Button>) : null}
        {canReview ? (<Button asChild>
      ...
      <ConfirmDialog open={cancelOpen} onOpenChange={setCancelOpen} title="Cancel this pre-order" description="Tell us why you are cancelling (up to 500 characters)." confirmLabel="Confirm cancel" destructive loading={cancelMutation.isPending} onConfirm={() => {
            if (!reason.trim() || reason.length > 500) {
                toast.error('Invalid cancellation reason');
                return;
            }
            cancelMutation.mutate({ reason, version: order.version }, {
        ...
        <Textarea value={reason} onChange={(e) => setReason(e.target.value)} maxLength={500} placeholder="e.g. Schedule conflict…"/>
      </ConfirmDialog>
    </div>);
}
