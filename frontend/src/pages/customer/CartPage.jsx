...
export default function CartPage() {
        ...
        for (const product of refreshQuery.data) {
            ...
            if (cartItem.price !== product.price ||
                ...
                useCartStore.setState({
                    items: useCartStore.getState().items.map((i) => i.product_id === product.id
                        ? {
                            ...i,
                            price: product.price,
    ...
    const conflictingFarmers = Object.keys(grouped).filter((farmerId) => openOrdersQuery.data?.results.some((o) => o.farmer.id === Number(farmerId)));
    const total = items
        .filter((i) => i.is_available)
        .reduce((sum, i) => sum + moneyToNumber(i.price) * i.quantity, 0);
    if (items.length === 0) {
        return (<div className="cart-page">
        <PageHeader eyebrow="Ready to reserve" title="Your cart"/>
        <EmptyState title="Your cart is empty" description="Browse stalls and reserve produce to get started." actionLabel="Browse produce" onAction={() => {
                window.location.href = '/products';
            }}/>
      </div>);
    }
    return (<div className="cart-page">
      <PageHeader eyebrow="Ready to reserve" title="Your cart" description="Items from each stall become a separate pre-order at checkout." actions={<Button variant="outline" size="sm" onClick={() => clear()}>
            Empty cart
          </Button>}/>

      {(wouldExceedTotal || conflictingFarmers.length > 0) && (<div className="cart-page__warn">
          ...
          <div>
            ...
            {conflictingFarmers.length > 0 ? (<p className="cart-page__warn-note">
      ...
      <div className="cart-page__groups">
        {Object.entries(grouped).map(([farmerId, farmerItems]) => {
            const subtotal = farmerItems
                .filter((i) => i.is_available)
                .reduce((sum, i) => sum + moneyToNumber(i.price) * i.quantity, 0);
            return (<section key={farmerId} className="cart-page__group">
              <h2 className="cart-page__group-title">{farmerItems[0]?.farmer_name}</h2>
              <ul className="cart-page__items">
                {farmerItems.map((item) => (<li key={item.product_id} className="cart-page__item">
                    {item.image ? (<LazyImage src={item.image} alt="" className="cart-page__item-thumb"/>) : (<div className="cart-page__item-thumb--empty"/>)}
                    <div className="cart-page__item-body">
                      ...
                      {!item.is_available ? (<p className="cart-page__item-unavailable">Unavailable</p>) : null}
                    </div>
                    <QuantityStepper value={item.quantity} disabled={!item.is_available} onChange={(qty) => updateQuantity(item.product_id, qty)}/>
      ...
      <div className="cart-page__footer">
        <div className="cart-page__footer-row">
          ...
          <Button asChild size="lg" disabled={wouldExceedTotal || conflictingFarmers.length > 0}>
