...
function readCreateOrderDetails(data) {
    ...
    for (const item of details) {
        if (!item || typeof item !== 'object')
            continue;
        if (!('code' in item) || !('message' in item))
            continue;
...
function stepClass(step, current) {
    if (step === current)
        return 'checkout-page__step checkout-page__step--active';
    if (step < current)
        return 'checkout-page__step checkout-page__step--done';
...
export default function CheckoutPage() {
    ...
    if (farmerIds.length === 0) {
        ...
        <EmptyState title="Nothing ready to check out" description="Add in-stock produce to your cart, then come back here." actionLabel="Back to cart" onAction={() => navigate('/app/cart')}/>
      </div>);
    }
    return (<div className="checkout-page">
      <PageHeader eyebrow="Almost there" title="Checkout" description="Choose a pickup slot and pay at the stall — no online payment."/>

      <div className="checkout-page__steps">
        {['Review', 'Pickup slot', 'Confirm'].map((label, index) => {
            const n = index + 1;
            return (<div key={label} className={stepClass(n, step)}>
              {n}. {label}
            </div>);
        })}
      </div>

      {step === 1 ? (<div className="checkout-page__panels">
          {farmerIds.map((farmerId) => (<div key={farmerId} className="checkout-page__panel">
              <h3 className="checkout-page__panel-title">
                {grouped[farmerId]?.[0]?.farmer_name}
              </h3>
              <ul className="checkout-page__line-list">
                {grouped[farmerId]?.map((item) => (<li key={item.product_id} className="checkout-page__line-row">
                    <span>
      ...
      {step === 2 ? (<div className="checkout-page__farmer-block">
          {farmerIds.map((farmerId, index) => {
                const options = pickupQueries[index]?.data ?? [];
                return (<div key={farmerId}>
                <h3 className="checkout-page__farmer-heading">
                  {grouped[farmerId]?.[0]?.farmer_name}
                </h3>
                {pickupQueries[index]?.isLoading ? (<p className="checkout-page__loading-hint">Loading pickup slots…</p>) : (<TimeSlotPicker options={options} value={slots[farmerId] ?? null} horizonDays={config.booking_horizon_days} onChange={(value) => setSlots((prev) => ({ ...prev, [farmerId]: value }))}/>)}
                <Textarea className="checkout-page__note-field" placeholder="Optional note for the stall" value={notes[farmerId] ?? ''} onChange={(e) => setNotes((prev) => ({ ...prev, [farmerId]: e.target.value }))}/>
              </div>);
            })}
      ...
      {step === 3 ? (<div className="checkout-page__panels">
          {farmerIds.map((farmerId) => {
                const slot = slots[farmerId];
                const subtotal = grouped[farmerId]?.reduce((s, i) => s + moneyToNumber(i.price) * i.quantity, 0) ?? 0;
                return (<div key={farmerId} className="checkout-page__panel">
                <h3 className="checkout-page__panel-title">
                  {grouped[farmerId]?.[0]?.farmer_name}
                </h3>
                <p className="checkout-page__slot-label">{slot?.label}</p>
                <p className="checkout-page__subtotal">{formatVnd(subtotal)}</p>
                {notes[farmerId] ? (<p className="checkout-page__note-text">Note: {notes[farmerId]}</p>) : null}
              </div>);
            })}
          ...
          <div className="page-primitive__actions-between checkout-page__panels-actions">
            ...
            <Button loading={createMutation.isPending} onClick={() => {
                void createMutation
                    .mutateAsync({
      ...
      <Dialog open={errorOpen} onOpenChange={setErrorOpen}>
        <DialogContent>
          ...
          <ul className="checkout-page__error-list">
            {errorDetails.map((err, index) => (<li key={`${err.code}-${index}`} className="checkout-page__error-item">
