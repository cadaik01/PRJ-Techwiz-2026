...
export default function CustomerDashboardPage() {
    ...
    return (<div className="customer-dashboard-page">
      <PageHeader eyebrow="Your market day" title="Your overview" description="Track open pre-orders and the next pickup on your calendar."/>

      <div className="customer-dashboard-page__stats">
        {stats.map((item) => (<Card key={item.label} className="customer-dashboard-page__stat-card">
      ...
      <div className="customer-dashboard-page__main">
        <Card className="customer-dashboard-page__upcoming-wide">
          ...
          <CardContent className="page-primitive__stack-3">
            {data?.upcoming.length ? (data.upcoming.map((order) => (<div key={order.id}>
                  <OrderCard order={order}/>
                  {order.id === latest?.id ? (<Countdown className="customer-dashboard-page__countdown" targetIso={order.pickup_start_at} label="Time until pickup"/>) : null}
                </div>))) : (<p className="page-primitive__muted-sm">
        ...
        <div className="customer-dashboard-page__sidebar">
          ...
          <Card>
            ...
            <CardContent className="page-primitive__stack-3">
              {data?.recent_notifications.map((n) => (<div key={n.id} className="customer-dashboard-page__notif-item">
              ...
              <Button asChild size="sm" variant="link" className="customer-dashboard-page__link-all">
