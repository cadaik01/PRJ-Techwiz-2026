...
import {
  ...
  useFarmerOrders,
  usePickingList,
} from '@/features/farmer/hooks/useFarmerOrders';
...
export default function FarmerOrdersPage() {
  ...
  const tab: TabId = isTabId(tabParam) ? tabParam : 'pending';
  const [q, setQ] = useState(params.get('q') ?? '');
  const [pickupDate, setPickupDate] = useState(
    params.get('pickup_date') ?? new Date().toISOString().slice(0, 10),
  );
  const [mode, setMode] = useState<'orders' | 'picking'>('orders');
  ...
  const countMap: Partial<Record<TabId, number>> = useMemo(() => {
