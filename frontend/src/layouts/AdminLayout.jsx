...
import { ClipboardList, FolderTree, Home, LayoutDashboard, Menu, ShieldAlert, Store, Users, Warehouse, FileBarChart, Megaphone, ScrollText, Settings } from 'lucide-react';
...
function SideNav({ collapsed }) {
    return (<nav className="admin-layout__nav">
      {navItems.map((item) => (<NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => cn('admin-layout__nav-link', isActive && 'is-active', collapsed && 'admin-layout__nav-link--collapsed')} title={item.label}>
...
export function AdminLayout() {
    ...
    return (<div className="admin-layout">
      ...
      <aside className={cn('admin-layout__sidebar', collapsed
            ? 'admin-layout__sidebar--collapsed'
            : 'admin-layout__sidebar--expanded')}>
        <div className="admin-layout__sidebar-head">
          {!collapsed ? (<Link to="/admin" className="admin-layout__sidebar-brand">
        ...
        <header className="glass admin-layout__header">
          ...
          <div className="admin-layout__header-actions">
            ...
            <Button asChild variant="ghost" size="icon" aria-label="Home">
              <Link to="/">
                <Home className="admin-layout__nav-icon"/>
              </Link>
            </Button>
            <UserMenu />
          </div>
