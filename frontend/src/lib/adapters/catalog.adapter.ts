...
function readDays(value: unknown): DayOfWeek[] {
...
export function adaptProduct(raw: unknown): ProductCard | null {
...
export function adaptProductDetail(raw: unknown): ProductDetail | null {
...
export function adaptFarmerProduct(raw: unknown): FarmerProduct | null {
...
export function adaptProductPage(raw: unknown): PaginatedData<ProductCard> {
  const page = adaptPaginated<unknown>(raw);
  return {
    ...page,
...
export function adaptFarmerProductPage(raw: unknown): PaginatedData<FarmerProduct> {
  const page = adaptPaginated<unknown>(raw);
  return {
    ...page,
    results: page.results.flatMap((item) => {
...
export function adaptMarket(raw: unknown): MarketSummary | null {
  if (!isRecord(raw) || typeof raw.id !== 'number' || typeof raw.name !== 'string') {
    return null;
  }
...
export function adaptMarketDetail(raw: unknown): Market | null {
...
export function adaptMarketPage(raw: unknown): PaginatedData<MarketSummary> {
  const page = adaptPaginated<unknown>(raw);
  return {
    ...page,
    results: page.results.flatMap((item) => {
...
export function adaptFarmerSummary(raw: unknown): FarmerSummary | null {
...
export function adaptFarmerPage(raw: unknown): PaginatedData<FarmerSummary> {
  const page = adaptPaginated<unknown>(raw);
  return {
    ...page,
    results: page.results.flatMap((item) => {
