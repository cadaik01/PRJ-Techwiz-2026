...
function buildDisplayName(raw) {
    if (raw.display_name && raw.display_name.trim())
        return raw.display_name.trim();
    if (raw.full_name && raw.full_name.trim())
        return raw.full_name.trim();
    if (raw.stall_name && raw.stall_name.trim())
        return raw.stall_name.trim();
...
function readFarmerStatus(raw) {
    if (raw.farmer_status)
        return raw.farmer_status;
    if (raw.farmer_profile?.status)
        return raw.farmer_profile.status;
    return null;
}
...
export function stripConfirmPassword(payload) {
